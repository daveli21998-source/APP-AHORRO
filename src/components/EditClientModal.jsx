...
import { updateCliente, getLugares, deleteLugar } from '../db';

export default function EditClientModal({ cliente, onClose, onSaved }) {
    const [form, setForm] = useState({
        nombre: cliente.nombre || '',
        puesto: cliente.puesto || '',
        pasaje: cliente.pasaje || '',
        lugar: cliente.lugar || '',
        tipoAhorro: cliente.tipoAhorro || 'normal',
        telefono: cliente.telefono || '',
        montoNormal: cliente.montoNormal || '',
        montoPuesto: cliente.montoPuesto || '',
    });
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
            <div className="modal-sheet" role="dialog" aria-modal="true">
                ...
                <div style={{ display: 'flex', alignItems: 'center', marginBottom: 20 }}>
                    <div className="modal-title" style={{ margin: 0, flex: 1 }}>Editar Cliente</div>
                ...
                <form onSubmit={handleSubmit}>
                    <div className="form-group">
                        <label className="form-label" htmlFor="edit-nombre">Nombre completo <Req /></label>
                        <input id="edit-nombre" name="nombre" className="form-input"
                            placeholder="Ej: Mario López" value={form.nombre} onChange={handleChange} autoFocus style={inp('nombre')} />
                        {errors.nombre && <Err msg={errors.nombre} />}
                    </div>

                    <div className="form-row">
                        <div className="form-group">
                            <label className="form-label" htmlFor="edit-puesto">Puesto <Req /></label>
                            <input id="edit-puesto" name="puesto" className="form-input"
                                placeholder="Ej: 1" value={form.puesto} onChange={handleChange} style={inp('puesto')} />
                            {errors.puesto && <Err msg={errors.puesto} />}
                        </div>
                        <div className="form-group">
                            <label className="form-label" htmlFor="edit-pasaje">N° Pasaje <Req /></label>
                            <input id="edit-pasaje" name="pasaje" className="form-input"
                                placeholder="Ej: 2" value={form.pasaje} onChange={handleChange} style={inp('pasaje')} />
                    ...
                    <div className="form-group">
                        ...
                        <LugarSelector
                            value={form.lugar}
                            lugares={lugares}
                            onChange={val => { setForm(f => ({ ...f, lugar: val })); setErrors(e => ({ ...e, lugar: '' })); }}
    ...
    return (
        <div ref={ref} style={{ position: 'relative' }}>
            <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
                <ModeTab active={mode === 'pick'} onClick={() => { setMode('pick'); onChange(value); setOpen(false); }}>
                    📋 Seleccionar
                </ModeTab>
                <ModeTab active={mode === 'write'} onClick={() => { setMode('write'); setOpen(false); }}>
                    ...
                    {open && (
                        <div style={{
                            ...
                            <div style={{ padding: '10px 12px', borderBottom: '1px solid var(--border-2)' }}>
                                <input className="form-input" placeholder="Buscar lugar..."
                            ...
                            <div style={{ maxHeight: 200, overflowY: 'auto' }}>
                                {filtered.map(lugar => (
                                    <div key={lugar}
                                        onClick={() => { onChange(lugar); setQuery(''); setOpen(false); }}
                                        style={{
                                        ...
                                        onMouseLeave={e => { if (value !== lugar) e.currentTarget.style.background = value === lugar ? 'var(--primary-soft)' : 'transparent'; }}
                                    >
                                        <span>📍</span> <span style={{ flex: 1 }}>{lugar}</span>
                                        {value === lugar && <span style={{ marginLeft: 'auto' }}>✓</span>}
                                        <button
                                            type="button"
...
function Err({ msg }) { return <div style={{ color: 'var(--danger)', fontSize: 11, fontWeight: 600, marginTop: 4 }}>⚠️ {msg}</div>; }
